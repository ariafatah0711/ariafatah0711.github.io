...
(function () {
  function initProjects() {
    // If projects UI is not present on this page, just no-op.
    if (!document.getElementById("projectModal") && !document.querySelector(".projects-gallery")) {
      return;
    }

    // Avoid binding global listeners multiple times under PJAX
    if (window.__projectsBound !== true) {
      window.__projectsBound = true;
      bindProjectsOnce();
    } else {
      // Already bound, but rebind modal controls (element refs changed after PJAX)
      try {
        rebindModalControls();
      } catch (e) {}
    }

    // Refresh any repo badges in the newly swapped DOM
    try {
      refreshProjectRepoBadges();
    } catch (e) {}
  }
  ...
  function bindProjectsOnce() {
    ...
    function refreshProjectRepoBadges() {
      document.querySelectorAll(".github-stars").forEach(function (el) {
        var repo = el.getAttribute("data-repo");
        if (repo) fetchStarCount(repo, el);
        else clearRepoStatsOnElement(el);
      });
    }

    window.refreshProjectRepoBadges = refreshProjectRepoBadges;

    // Rebind modal controls (safe for PJAX - called on every init)
    function rebindModalControls() {
      projectModal = document.getElementById("projectModal");
      modalImg = document.getElementById("projectModalImg");
      modalTitle = document.getElementById("projectModalTitle");
      modalDescription = document.getElementById("projectModalDescription");
      modalCounter = document.getElementById("projectModalCounter");
      prevBtn = document.getElementById("projectPrev");
      nextBtn = document.getElementById("projectNext");
      closeBtn = document.getElementById("projectClose");
      sourceBtn = document.getElementById("projectModalSource");
      demoBtn = document.getElementById("projectModalDemo");
      starsEl = document.getElementById("projectModalStars");
    ...
    // click handler (event delegation - survives PJAX)
